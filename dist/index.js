...
var parse = __nccwpck_require__(925);
...
const isValidReleaseType = (s) => RELEASE_TYPES.includes(s);
...
try {
    ...
    const increment = core.getInput('increment', REQUIRED);
    if (!isValidReleaseType(increment))
        throw new Error(`Invalid release type "${increment}".\n` +
    ...
    const lines = (0,external_fs_.readFileSync)(version_file, 'utf8').split('\n');
    ...
    const current = parse_default()(version_regex.exec(lines[n])[1]);
...
Updated "${version_file}" preview:`);
    ...
    lines
        ...
        .forEach((v, i) => console.log(i == n ? v : source.green(v)));
    console.log('────────────────────────────────────────────────────────────');
    (0,external_fs_.writeFileSync)(version_file, lines.join('\n'));
