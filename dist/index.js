...
/* harmony import */ var semver_functions_parse__WEBPACK_IMPORTED_MODULE_2__ = __nccwpck_require__(925);
/* harmony import */ var semver_functions_parse__WEBPACK_IMPORTED_MODULE_2___default = /*#__PURE__*/__nccwpck_require__.n(semver_functions_parse__WEBPACK_IMPORTED_MODULE_2__);



// prettier-ignore
const RELEASE_TYPES = ['major', 'premajor', 'minor', 'preminor', 'patch', 'prepatch', 'prerelease'];
function assertRelease(s) {
    const msg = `Invalid release type "${s}".\nUse one of ${RELEASE_TYPES}`;
    if (!RELEASE_TYPES.includes(s))
        throw new Error(msg);
}
function inc(sv, rt, id) {
...
function getAllInput() {
    ...
    const input = {
        increment: (0,_actions_core__WEBPACK_IMPORTED_MODULE_0__.getInput)('increment', R),
        identifier: (0,_actions_core__WEBPACK_IMPORTED_MODULE_0__.getInput)('identifier'),
        ver: {
            file: (0,_actions_core__WEBPACK_IMPORTED_MODULE_0__.getInput)('version_file', R),
            regex: new RegExp((0,_actions_core__WEBPACK_IMPORTED_MODULE_0__.getInput)('version_regex', R)),
        },
    };
    assertRelease(input.increment);
    return input;
}
function preview(lines, delta, range = 3) {
...
try {
    ...
    const lines = (0,fs__WEBPACK_IMPORTED_MODULE_1__.readFileSync)(ver.file, 'utf8').split('\n');
    const n = lines.findIndex((v) => ver.regex.test(v));
    if (n === -1)
        throw new Error(`Regex doesn't match any line`);
    // won't throw because regex has matched previously already
    const current = semver_functions_parse__WEBPACK_IMPORTED_MODULE_2___default()(ver.regex.exec(lines[n])[1]);
    const next = inc(semver_functions_parse__WEBPACK_IMPORTED_MODULE_2___default()(current.version), increment, identifier);
...
Updated "${ver.file}" preview:`);
    preview(lines, n);
    (0,fs__WEBPACK_IMPORTED_MODULE_1__.writeFileSync)(ver.file, lines.join('\n'));
    (0,_actions_core__WEBPACK_IMPORTED_MODULE_0__.setOutput)('version', next.version);
}
catch (err) {
